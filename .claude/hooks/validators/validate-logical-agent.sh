#!/bin/bash
# Validator for logical-agent (Stage 5.5)
# Validates Logic Verification Report format with Issues Found
#
...
if ! echo "$OUTPUT" | grep -qi "Logic.*Verification\|Verification.*Report\|## Logic\|### Logic" 2>/dev/null; then
    ERRORS+=("Resolve this Format error in logical-agent output: Missing required section - Logic Verification Report")
fi

if ! echo "$OUTPUT" | grep -qi "Issues Found\|Issues.*:\|No Issues\|### Issues" 2>/dev/null; then
    ERRORS+=("Resolve this Format error in logical-agent output: Missing required section - Issues Found")
fi

if ! echo "$OUTPUT" | grep -qi "Files Reviewed\|Code Reviewed\|Reviewed" 2>/dev/null; then
    WARNINGS+=("Missing section: Files Reviewed (recommended)")
fi

if ! echo "$OUTPUT" | grep -qi "Edge Cases\|Boundary\|edge case" 2>/dev/null; then
    WARNINGS+=("Missing section: Edge Cases analysis (recommended)")
fi

if ! echo "$OUTPUT" | grep -qi "Recommendation\|Status\|Verdict" 2>/dev/null; then
    WARNINGS+=("Missing section: Recommendation/Status (recommended)")
fi
