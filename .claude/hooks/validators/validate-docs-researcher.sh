#!/bin/bash
# Validator for docs-researcher agent (Stage 3)
# Validates Documentation Report format with Libraries Researched
#
...
if ! echo "$OUTPUT" | grep -qi "Documentation Report\|## Documentation\|### Documentation" 2>/dev/null; then
    ERRORS+=("Resolve this Format error in docs-researcher output: Missing required section - Documentation Report")
fi

if ! echo "$OUTPUT" | grep -qi "Libraries Researched\|### Libraries\|Library.*Version" 2>/dev/null; then
    ERRORS+=("Resolve this Format error in docs-researcher output: Missing required section - Libraries Researched")
fi

if ! echo "$OUTPUT" | grep -qi "API Syntax\|Current Syntax\|Correct Syntax" 2>/dev/null; then
    WARNINGS+=("Missing section: API Syntax examples (recommended)")
fi

if ! echo "$OUTPUT" | grep -qi "Best Practices\|best practice" 2>/dev/null; then
    WARNINGS+=("Missing section: Best Practices (recommended)")
fi

if ! echo "$OUTPUT" | grep -qi "Implementation Guidance\|Guidance" 2>/dev/null; then
    WARNINGS+=("Missing section: Implementation Guidance (recommended)")
fi

if ! echo "$OUTPUT" | grep -qi "Pitfalls\|Warnings\|Warning" 2>/dev/null; then
    WARNINGS+=("Missing section: Pitfalls/Warnings (recommended)")
fi
